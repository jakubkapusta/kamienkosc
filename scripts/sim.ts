// Balance report: npm run sim -- [runsPerClass] [skill] [short|long]
import { BAL } from '../src/game/balance';
import { LENGTHS, type RunLen } from '../src/game/map';
...
const len = (process.argv[4] ?? 'short') as RunLen;
const ROWS = LENGTHS[len].rows;
const pct = (a: number, b: number) => (b ? `${((a / b) * 100).toFixed(1)}%` : '—');
const avg = (xs: number[]) => (xs.length ? xs.reduce((a, b) => a + b, 0) / xs.length : 0);

for (const skill of skills) {
  console.log(`\n=== skill ${skill} · ${runs} wypraw na klasę · ${len} (${ROWS} pięter) ===`);
  const all: RunOut[] = [];
  for (const cls of CLASS_IDS) {
    const res: RunOut[] = [];
    for (let i = 0; i < runs; i++) res.push(await simRun(cls, 1000 + i * 7919, skill, len));
    all.push(...res);
  ...
  console.log(`\nGdzie giną (piętro 1–${ROWS}, rodzaj):`);
